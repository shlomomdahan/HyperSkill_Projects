...
def validate_move(row, col):
    ...
    if sz[0] > col >= 0 and sz[1] > row >= 0 and board[row][col] in valid:
        return True


# def solver(board, row, col, rows, cols, counter):
    ...
    #     new_y = col + move_y[i]
    #
    #     if validate_move(new_y, new_x):
    #         board[new_y][new_x] = str(counter)
    #         if solver(board, new_x, new_y, rows, cols, counter+1):
    #             return True
    #         if cell_size == 1:
    #             board[new_y][new_x] = '_'
    #         elif cell_size == 2:
    #             board[new_y][new_x] = '__'
    #         elif cell_size == 3:
    #             board[new_y][new_x] = '___'
    #
    # return False


def solver(rows, cols, board, curr_x, curr_y, pos):
    ...
    for i in range(8):
        ...
        if (validate_move(new_x, new_y)):
            ...
            if (solver(rows, cols, board, new_x, new_y, pos + 1)):
                return True
...
if __name__ == "__main__":
    ...
    response = ask_player()
    if response == 'no':
        if solver(sz[1], sz[0], board, ans[1], ans[0], 1):
            print("Here's the solution!")
            print_board(sz[1], sz[0])
        elif not solver(sz[1], sz[0], board, ans[1], ans[0], 1):
            print("No solution exists!")
    elif response == 'yes':
        if not solver(sz[1], sz[0], board, ans[1], ans[0], 1):
            print("No solution exists!")
        elif solver(sz[1], sz[0], board, ans[1], ans[0], 1):
